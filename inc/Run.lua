...
print('\n\27[1;36m￤ تم كتابه ايدي المطور , سوف يتم تشغيل البوت علي سورس دوكس الان .\n￤Success Save USERID : \27[0;32m['..SUDO_USER..']\n\27[0;39;49m')
dox = Token:match("(%d+)")
redis:set(dox..":VERSION",1)
redis:set(dox..":SUDO_ID:",SUDO_USER)
redis:set(dox..":DataCenter:",'German')
redis:set(dox..":UserNameBot:",BOT_User)
redis:set(dox..":NameBot:",BOT_NAME)
redis:hset(dox..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3dox_INSTALL","Yes")
info = {}
...
Bot_User = redis:get(dox..":UserNameBot:")
SUDO_ID = tonumber(redis:get(dox..":SUDO_ID:"))
SUDO_USER = redis:hgetall(dox..'username:'..SUDO_ID).username
version = redis:get(dox..":VERSION")
DataCenter = redis:get(dox..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..dox..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(dox..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(dox..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(dox..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(dox..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(dox..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(dox..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(dox..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(dox..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(dox..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(dox..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(dox..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(dox..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(dox..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(dox..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(dox..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس دوكس ⇐ @SourceDoX
...
if msg.GroupActive and redis:get(dox..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(dox..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(dox..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(dox..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.dox then 
if msg.text and ScriptFile.idox then
for k, dox in pairs(ScriptFile.dox) do
local SearchText = msg.text:match(dox)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.idox(msg,{msg.text:match(dox)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",dox.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.ddox then
if ScriptFile.ddox(msg) == false then
return false
end
...
if Plug.dox then 
if msg.text and Plug.idox then
for k, dox in pairs(Plug.dox) do
local SearchText = msg.text:match(dox)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(dox..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(dox..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.idox(msg,{msg.text:match(dox)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",dox.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source dox")
end 

end
else
print("The File Script.lua Not Runing in The Source dox")
...
function tdcli_update_callback(data)
	...
	redis:srem(dox..'users',msg.sender_user_id_)
	end
	...
	EditMsg(data.message_.chat_id_,data.message_.id_,'10% -  D         ')
	EditMsg(data.message_.chat_id_,data.message_.id_,'20% - D           |')
	download_file('https://raw.githubusercontent.com/daaarkkk0012512000/dox/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - D  O       ')
	download_file('https://raw.githubusercontent.com/daaarkkk0012512000/dox/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - D  O   ')
	download_file('https://raw.githubusercontent.com/daaarkkk0012512000/dox/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - D  O    ')
	download_file('https://raw.githubusercontent.com/daaarkkk0012512000/dox/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/daaarkkk0012512000/dox/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/daaarkkk0012512000/dox/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'100% - D  O  X..🖤\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل سورس دوكس بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	download_file('https://raw.githubusercontent.com/daaarkkk0012512000/dox/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/daaarkkk0012512000/dox/main/plugins/games.lua','./plugins/games.lua')
	sendMsg(msg.chat_id_,msg.id_,'• {* تم تحديث سورس دوكس بنجاح  *} .\n\n• { Bot is Update » }',nil,function(arg,data)
	dofile("./inc/Run.lua")
	...
	mmd = redis:get(dox..":SUDO_ID:")
redis:hset(dox..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(dox..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(dox..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(dox..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(dox..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(dox..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(dox..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(dox..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(dox..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(dox..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(dox..'group:add'..data.chat_.id_) then
	redis:set(dox..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(dox..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(dox..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(dox..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	dox..'group:name'..k,IDS.Title,
	dox..'num_msg_dox'..k,5,
	dox..'group:add'..k,true,
	dox..'lock_link'..k,true,
	dox..'lock_spam'..k,true,
	dox..'lock_webpage'..k,true,
	dox..'lock_markdown'..k,true,
	dox..'lock_flood'..k,true,
	dox..'lock_bots'..k,true,
	dox..'mute_forward'..k,true,
	dox..'mute_contact'..k,true,
	dox..'mute_document'..k,true,
	dox..'mute_inline'..k,true,
	dox..'lock_username'..k,true,
	dox..'replay'..k,true
	)
	redis:sadd(dox..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(dox..'username:'..ID,'username',user)
	redis:sadd(dox..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(dox..'username:'..ID,'username',user)
	redis:sadd(dox..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(dox..'username:'..ID,'username',user)
	redis:sadd(dox..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(dox..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(dox..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(dox..'username:'..data.user_.id_).username
	...
	redis:hset(dox..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
